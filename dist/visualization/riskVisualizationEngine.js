...
exports.RiskVisualizationEngine = void 0;
...
class RiskVisualizationEngine {
    ...
    initializeContainer() {
        ...
        logger_1.logger.info('Risk visualization container initialized');
    }
    createRiskNetworkMap(riskData) {
        logger_1.logger.info('Creating interactive risk network map with D3.js...');
        const nodes = riskData.map((data, index) => ({
        ...
        const links = this.generateRiskLinks(nodes);
        ...
        this.simulation = d3.forceSimulation(nodes)
            ...
            .force('collision', d3.forceCollide().radius((d) => this.sizeScale(d.riskScore) + 2));
        ...
        const node = g.append('g')
            ...
            .data(nodes)
            .enter().append('circle')
            .attr('r', (d) => this.sizeScale(d.riskScore))
            .attr('fill', (d) => this.colorScale(d.riskScore))
            .attr('stroke', '#fff')
            .attr('stroke-width', 2)
        ...
        const labels = g.append('g')
            ...
            .data(nodes)
            .enter().append('text')
            .text((d) => `${d.endpoint.split('/').pop()} (${d.method})`)
        ...
        node
            ...
            node.style('opacity', (n) => this.isConnected(d, n, links) ? 1 : 0.3);
        ...
        this.simulation.on('tick', () => {
            ...
            labels
                .attr('x', (d) => d.x)
                .attr('y', (d) => d.y + this.sizeScale(d.riskScore) + 15);
        });
        // Add legend
        this.createLegend();
        logger_1.logger.info(`Risk network map created with ${nodes.length} nodes and ${links.length} links`);
    }
    createRiskHeatmap(riskData) {
        logger_1.logger.info('Creating risk heatmap visualization...');
        ...
        logger_1.logger.info(`Heatmap created with ${riskData.length} data points`);
    }
    createRiskTimeline(timeSeriesData) {
        logger_1.logger.info('Creating risk timeline visualization...');
        const margin = this.config.margin;
        const width = this.config.width - margin.left - margin.right;
        const height = this.config.height - margin.top - margin.bottom;
        this.svg.selectAll('*').remove();
        ...
        const xScale = d3.scaleTime()
            .domain(d3.extent(timeSeriesData, (d) => d.timestamp))
            .range([0, width]);
        ...
        logger_1.logger.info(`Timeline created with ${timeSeriesData.length} data points across ${categories.length} categories`);
    }
    createMetricsDashboard(metrics) {
        logger_1.logger.info('Creating metrics dashboard...');
        this.svg.selectAll('*').remove();
        const cardWidth = 180;
        ...
        cards.append('text')
            ...
            .text((d) => d.label);
        logger_1.logger.info(`Metrics dashboard created with ${metricsData.length} metric cards`);
    }
    generateRiskLinks(nodes) {
        const links = [];
    ...
    calculateEndpointSimilarity(endpointA, endpointB) {
    ...
    isConnected(nodeA, nodeB, links) {
        return nodeA.id === nodeB.id || links.some(link => (link.source === nodeA.id && link.target === nodeB.id) ||
    ...
    onNodeClick(node) {
        logger_1.logger.info(`Node clicked: ${node.endpoint} (${node.method})`);
    ...
    createTimelineLegend(categories, colorScale) {
        const legend = this.svg.append('g')
    ...
    updateVisualization(newData) {
        logger_1.logger.info('Updating visualization with new data...');
    ...
    exportVisualization(format) {
        if (format === 'svg') {
            ...
            logger_1.logger.warn('PNG export not implemented yet');
            return '';
        }
    }
    destroy() {
        ...
        logger_1.logger.info('Visualization engine destroyed');
    }
}
exports.RiskVisualizationEngine = RiskVisualizationEngine;
//# sourceMappingURL=riskVisualizationEngine.js.map