...
exports.RealTimeApiScanner = void 0;
const endpointDiscovery_1 = require("../discovery/endpointDiscovery");
const logger_1 = require("../utils/logger");
class RealTimeApiScanner {
    constructor(riskEngine, discoveryEngine, authTester, parameterScanner) {
        this.dashboardServer = null;
        this.activeScan = null;
        this.scanCallbacks = new Map();
        this.riskEngine = riskEngine;
        this.discoveryEngine = discoveryEngine || new endpointDiscovery_1.EndpointDiscovery({ baseUrl: '', authMethod: 'none' });
        this.authTester = authTester;
        this.parameterScanner = parameterScanner;
    }
    setDashboardServer(dashboardServer) {
        this.dashboardServer = dashboardServer;
    }
    async startRealTimeScan(request) {
        const scanId = `scan_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
        logger_1.logger.info(`Starting real-time API scan: ${scanId}`, { targetUrl: request.targetUrl });
        this.activeScan = {
            scanId,
        ...
        this.performRealTimeScan(request, scanId).catch(error => {
            logger_1.logger.error(`Scan ${scanId} failed: ${error.message}`);
            this.broadcastScanUpdate('error', { scanId, error: error.message });
        });
        return scanId;
    }
    async performRealTimeScan(request, scanId) {
        if (!this.activeScan)
            return;
        try {
            ...
            logger_1.logger.error(`Real-time scan failed: ${error.message}`);
            throw error;
        }
    }
    async performDiscoveryPhase(request) {
        logger_1.logger.info('Starting API discovery phase...');
        ...
        if (request.targetUrl.includes('jsonplaceholder')) {
            logger_1.logger.info('Detected JSONPlaceholder - using known endpoints');
            ...
            logger_1.logger.info(`Added ${jsonPlaceholderEndpoints.length} known JSONPlaceholder endpoints`);
        }
        for (let i = 0; i < request.scanMethods.length; i++) {
            ...
            try {
                ...
                switch (method) {
                    case 'swagger':
                        logger_1.logger.info(`Starting Swagger/OpenAPI discovery for ${request.targetUrl}`);
                        try {
                            const { SwaggerDiscovery } = await Promise.resolve().then(() => __importStar(require('../discovery/swaggerDiscovery')));
                            const target = {
                            ...
                            logger_1.logger.info(`Swagger discovery found ${endpoints.length} endpoints`);
                        }
                        catch (swaggerError) {
                            logger_1.logger.error(`Swagger discovery failed: ${swaggerError.message}`);
                            logger_1.logger.error(`Stack trace: ${swaggerError.stack}`);
                        }
                        break;
                    case 'crawl':
                        logger_1.logger.info(`Starting web crawling discovery for ${request.targetUrl}`);
                        try {
                            // Use basic web crawling approach
                            endpoints = await this.performBasicWebCrawling(request.targetUrl);
                            logger_1.logger.info(`Web crawling found ${endpoints.length} endpoints`);
                        }
                        catch (crawlError) {
                            logger_1.logger.error(`Web crawling failed: ${crawlError.message}`);
                            logger_1.logger.error(`Stack trace: ${crawlError.stack}`);
                        }
                        break;
                    case 'brute_force':
                        logger_1.logger.info(`Starting brute force discovery for ${request.targetUrl}`);
                        try {
                            // Use basic brute force approach
                            endpoints = await this.performBasicBruteForce(request.targetUrl);
                            logger_1.logger.info(`Brute force discovery found ${endpoints.length} endpoints`);
                        }
                        catch (bruteError) {
                            logger_1.logger.error(`Brute force discovery failed: ${bruteError.message}`);
                            logger_1.logger.error(`Stack trace: ${bruteError.stack}`);
                        }
                        break;
                }
                if (endpoints.length > 0) {
                    discoveryResults.push(...endpoints);
                    logger_1.logger.info(`Added ${endpoints.length} endpoints from ${method} discovery`);
                }
                ...
                this.updateScanProgress('discovery', methodProgress, `Found ${this.activeScan.discoveredEndpoints.length} endpoints via ${method}`);
                ...
                logger_1.logger.error(`${method} discovery failed with error: ${error.message}`);
                logger_1.logger.error(`Error stack: ${error.stack}`);
            }
        }
        logger_1.logger.info(`Discovery complete: ${this.activeScan.discoveredEndpoints.length} total endpoints found`);
    }
    getJSONPlaceholderEndpoints(baseUrl) {
        const endpoints = [];
        ...
        logger_1.logger.info(`JSONPlaceholder detection: Added ${endpoints.length} known endpoints`);
        return endpoints;
    }
    generateJSONPlaceholderParameters(path, method) {
        const parameters = [];
        ...
        if (['POST', 'PUT', 'PATCH'].includes(method)) {
            if (path.includes('/posts')) {
                parameters.push({ name: 'title', type: 'body', dataType: 'string', required: true, description: 'Post title' }, { name: 'body', type: 'body', dataType: 'string', required: true, description: 'Post content' }, { name: 'userId', type: 'body', dataType: 'integer', required: true, description: 'Author user ID' });
            }
            else if (path.includes('/comments')) {
                parameters.push({ name: 'name', type: 'body', dataType: 'string', required: true, description: 'Commenter name' }, { name: 'email', type: 'body', dataType: 'string', required: true, description: 'Commenter email' }, { name: 'body', type: 'body', dataType: 'string', required: true, description: 'Comment content' }, { name: 'postId', type: 'body', dataType: 'integer', required: true, description: 'Post ID' });
            }
            else if (path.includes('/albums')) {
                parameters.push({ name: 'title', type: 'body', dataType: 'string', required: true, description: 'Album title' }, { name: 'userId', type: 'body', dataType: 'integer', required: true, description: 'Owner user ID' });
            }
            else if (path.includes('/photos')) {
                parameters.push({ name: 'title', type: 'body', dataType: 'string', required: true, description: 'Photo title' }, { name: 'url', type: 'body', dataType: 'string', required: true, description: 'Photo URL' }, { name: 'thumbnailUrl', type: 'body', dataType: 'string', required: true, description: 'Thumbnail URL' }, { name: 'albumId', type: 'body', dataType: 'integer', required: true, description: 'Album ID' });
            }
            else if (path.includes('/todos')) {
                parameters.push({ name: 'title', type: 'body', dataType: 'string', required: true, description: 'Todo title' }, { name: 'completed', type: 'body', dataType: 'boolean', required: true, description: 'Completion status' }, { name: 'userId', type: 'body', dataType: 'integer', required: true, description: 'Owner user ID' });
            }
            else if (path.includes('/users')) {
                parameters.push({ name: 'name', type: 'body', dataType: 'string', required: true, description: 'Full name' }, { name: 'username', type: 'body', dataType: 'string', required: true, description: 'Username' }, { name: 'email', type: 'body', dataType: 'string', required: true, description: 'Email address' }, { name: 'phone', type: 'body', dataType: 'string', required: false, description: 'Phone number' }, { name: 'website', type: 'body', dataType: 'string', required: false, description: 'Website URL' });
            }
        }
        // Common query parameters for all GET requests
        if (method === 'GET' && !path.includes('?') && !path.includes('/1')) {
            parameters.push({ name: '_limit', type: 'query', dataType: 'integer', required: false, description: 'Limit results', example: 10 }, { name: '_start', type: 'query', dataType: 'integer', required: false, description: 'Start offset', example: 0 }, { name: '_sort', type: 'query', dataType: 'string', required: false, description: 'Sort field', example: 'id' }, { name: '_order', type: 'query', dataType: 'string', required: false, description: 'Sort order', example: 'asc' });
        }
        return parameters;
    }
    async performBasicWebCrawling(targetUrl) {
        const endpoints = [];
        try {
            ...
            for (let i = 0; i < commonApiPaths.length; i += maxConcurrent) {
                        ...
                        if (response.status < 500 && response.status !== 404) {
                            ...
                            if (this.isSwaggerEndpoint(path, contentType)) {
                                try {
                                    ...
                                    logger_1.logger.info(`Swagger discovery: Found ${swaggerEndpoints.length} endpoints from ${testUrl}`);
                                }
                                catch (swaggerError) {
                                    logger_1.logger.warn(`Failed to parse swagger from ${testUrl}: ${swaggerError}`);
                                }
                            }
                ...
                this.updateScanProgress('discovery', progress, `Web crawling: ${Math.min(i + maxConcurrent, commonApiPaths.length)}/${commonApiPaths.length} paths tested`);
            }
            logger_1.logger.info(`Enhanced web crawling completed: ${endpoints.length} endpoints found`);
        }
        catch (error) {
            logger_1.logger.warn(`Enhanced web crawling failed: ${error.message}`);
        }
        return endpoints;
    }
    isSwaggerEndpoint(path, contentType) {
    ...
    parseBasicSwaggerEndpoints(swaggerData, baseUrl) {
        const endpoints = [];
        try {
                    ...
                    if (typeof pathData === 'object' && pathData !== null) {
                        for (const [method, methodData] of Object.entries(pathData)) {
                            if (['get', 'post', 'put', 'delete', 'patch'].includes(method.toLowerCase())) {
                                ...
                                const parameters = this.extractSwaggerParameters(methodData, pathData);
                                endpoints.push({
                                    url: fullUrl,
            ...
            logger_1.logger.warn(`Failed to parse Swagger endpoints: ${error}`);
        }
        return endpoints;
    }
    extractSwaggerParameters(methodData, pathData) {
        const parameters = [];
            ...
            if (content['application/json']?.schema?.properties) {
                ...
                for (const [propName, propData] of Object.entries(properties)) {
                    parameters.push({
                        name: propName,
                        type: 'body',
                        dataType: propData.type || 'string',
                        required: methodData.requestBody.required || false,
                        example: propData.example
                    });
                }
            }
        }
    ...
    mapSwaggerParamType(swaggerType) {
        switch (swaggerType) {
    ...
    detectParametersFromEndpoint(path, response, hasParams) {
        const parameters = [];
        if (!hasParams)
            return parameters;
        ...
        if (path.includes('search')) {
            parameters.push({ name: 'q', type: 'query', dataType: 'string', required: true }, { name: 'limit', type: 'query', dataType: 'integer', required: false }, { name: 'offset', type: 'query', dataType: 'integer', required: false });
        }
        if (path.includes('upload')) {
            parameters.push({ name: 'file', type: 'body', dataType: 'file', required: true }, { name: 'filename', type: 'body', dataType: 'string', required: false });
        }
        ...
        if (path.includes('/api/')) {
            parameters.push({ name: 'page', type: 'query', dataType: 'integer', required: false }, { name: 'limit', type: 'query', dataType: 'integer', required: false }, { name: 'sort', type: 'query', dataType: 'string', required: false }, { name: 'filter', type: 'query', dataType: 'string', required: false });
        }
        return parameters;
    }
    generatePostParameters(path) {
        const parameters = [];
        if (path.includes('users')) {
            parameters.push({ name: 'name', type: 'body', dataType: 'string', required: true }, { name: 'email', type: 'body', dataType: 'string', required: true }, { name: 'password', type: 'body', dataType: 'string', required: true });
        }
        else if (path.includes('posts')) {
            parameters.push({ name: 'title', type: 'body', dataType: 'string', required: true }, { name: 'content', type: 'body', dataType: 'string', required: true }, { name: 'author_id', type: 'body', dataType: 'integer', required: true });
        }
        else if (path.includes('search')) {
            parameters.push({ name: 'query', type: 'body', dataType: 'string', required: true }, { name: 'filters', type: 'body', dataType: 'object', required: false });
        }
        else {
            // Generic parameters for unknown endpoints
            parameters.push({ name: 'data', type: 'body', dataType: 'object', required: true }, { name: 'id', type: 'body', dataType: 'string', required: false });
        }
        return parameters;
    }
    async performBasicBruteForce(targetUrl) {
        const endpoints = [];
        try {
            ...
            for (let i = 0; i < highValueEndpoints.length; i += maxConcurrency) {
                ...
                this.updateScanProgress('discovery', progress, `Brute force: ${processedCount}/${totalEndpoints} endpoints tested (${endpoints.length} found)`);
                // Small delay to prevent overwhelming the target
                await new Promise(resolve => setTimeout(resolve, 100));
            }
            logger_1.logger.info(`Fast brute force discovery completed: ${endpoints.length} endpoints found in ${totalEndpoints} tests`);
        }
        catch (error) {
            logger_1.logger.warn(`Optimized brute force discovery failed: ${error.message}`);
        }
        return endpoints;
    }
    detectAuthMethods(response) {
        const authMethods = [];
    ...
    generateBruteForceParameters(path, method) {
        const parameters = [];
        // Path-based parameter detection
        if (path.includes('/admin') || path.includes('/management')) {
            if (method === 'GET') {
                parameters.push({ name: 'page', type: 'query', dataType: 'integer', required: false }, { name: 'limit', type: 'query', dataType: 'integer', required: false });
            }
            else if (method === 'POST') {
                parameters.push({ name: 'action', type: 'body', dataType: 'string', required: true }, { name: 'data', type: 'body', dataType: 'object', required: false });
            }
        }
        if (path.includes('/login') || path.includes('/auth')) {
            if (method === 'POST') {
                parameters.push({ name: 'username', type: 'body', dataType: 'string', required: true }, { name: 'password', type: 'body', dataType: 'string', required: true }, { name: 'remember_me', type: 'body', dataType: 'boolean', required: false });
            }
        }
        if (path.includes('/upload')) {
            if (method === 'POST') {
                parameters.push({ name: 'file', type: 'body', dataType: 'file', required: true }, { name: 'directory', type: 'body', dataType: 'string', required: false }, { name: 'overwrite', type: 'body', dataType: 'boolean', required: false });
            }
        }
        if (path.includes('/search')) {
            if (method === 'GET') {
                parameters.push({ name: 'q', type: 'query', dataType: 'string', required: true }, { name: 'type', type: 'query', dataType: 'string', required: false }, { name: 'limit', type: 'query', dataType: 'integer', required: false }, { name: 'offset', type: 'query', dataType: 'integer', required: false });
            }
        }
        if (path.includes('/users') || path.includes('/user')) {
            if (method === 'GET') {
                parameters.push({ name: 'id', type: 'path', dataType: 'integer', required: false }, { name: 'username', type: 'query', dataType: 'string', required: false });
            }
            else if (method === 'POST') {
                parameters.push({ name: 'username', type: 'body', dataType: 'string', required: true }, { name: 'email', type: 'body', dataType: 'string', required: true }, { name: 'password', type: 'body', dataType: 'string', required: true }, { name: 'role', type: 'body', dataType: 'string', required: false });
            }
        }
        // Generic API parameters
        if (path.includes('/api/')) {
            if (method === 'GET') {
                parameters.push({ name: 'format', type: 'query', dataType: 'string', required: false }, { name: 'version', type: 'query', dataType: 'string', required: false });
            }
            else if (['POST', 'PUT', 'PATCH'].includes(method)) {
                parameters.push({ name: 'data', type: 'body', dataType: 'object', required: true }, { name: 'validate', type: 'body', dataType: 'boolean', required: false });
            }
        }
        return parameters;
    }
    async performAuthenticationPhase(request) {
        logger_1.logger.info('Starting authentication testing phase...');
        ...
        for (let i = 0; i < endpoints.length; i++) {
            ...
            this.updateScanProgress('auth_testing', progressBase, `Testing authentication: ${endpoint.url}`, endpoint.url);
            try {
                // Use the actual AuthenticationTester with correct method name
                logger_1.logger.info(`Testing authentication for ${endpoint.url}`);
                ...
                for (const authResult of authResults) {
                    if (authResult.vulnerable) { // Only include actual vulnerabilities
                        ...
                        logger_1.logger.info(`Found ${authResult.test.severity} auth vulnerability: ${authResult.test.type} in ${endpoint.url}`);
                    }
                }
                // Real-time update
                ...
                logger_1.logger.warn(`Auth testing failed for ${endpoint.url}: ${error.message}`);
            }
        }
        logger_1.logger.info(`Authentication testing complete: ${this.activeScan.vulnerabilities.length} vulnerabilities found`);
    }
    async performParameterTestingPhase(request) {
        logger_1.logger.info('Starting parameter vulnerability testing phase...');
        const endpoints = this.activeScan.discoveredEndpoints.filter(ep => ep.parameters && ep.parameters.length > 0);
        ...
        if (endpointsToTest.length === 0) {
            logger_1.logger.info('No endpoints with parameters found, skipping parameter testing');
            return;
        }
        logger_1.logger.info(`Testing parameters on ${endpointsToTest.length} endpoints (limited from ${endpoints.length} total)`);
        ...
        for (let i = 0; i < endpointsToTest.length; i++) {
            ...
            this.updateScanProgress('parameter_testing', progressBase, `Testing parameters: ${endpoint.url}`, endpoint.url);
            try {
                // Skip if we already have too many vulnerabilities
                if (vulnerabilitiesFound >= 50) {
                    logger_1.logger.info('Vulnerability limit reached, skipping remaining parameter tests');
                    break;
                }
                ...
                for (const vulnerability of parameterResults) {
                    ...
                    if (!vulnerabilitySet.has(vulnKey) && vulnerabilitiesFound < 50) {
                        ...
                        logger_1.logger.info(`Found ${vulnerability.severity} parameter vulnerability: ${vulnerability.type} in ${endpoint.url}[${vulnerability.parameter}]`);
                    }
                }
                this.activeScan.progress.vulnerabilitiesFound = this.activeScan.vulnerabilities.length;
                // Real-time update
                ...
                logger_1.logger.warn(`Parameter testing failed for ${endpoint.url}: ${error.message}`);
            }
        }
        logger_1.logger.info(`Parameter testing complete: ${this.activeScan.vulnerabilities.length} total vulnerabilities found`);
    }
    // Fast parameter testing for demo purposes
    async performFastParameterTest(endpoint) {
    ...
    async performRiskScoringPhase() {
        logger_1.logger.info('Starting AI/ML risk scoring phase...');
        ...
        for (let i = 0; i < vulnerabilities.length; i++) {
            ...
            this.updateScanProgress('risk_scoring', progressBase, `Calculating AI risk scores...`, vulnerability.endpoint);
            try {
                ...
                logger_1.logger.warn(`Risk scoring failed for vulnerability: ${error.message}`);
            }
        }
    }
    async completeScan() {
        logger_1.logger.info('Completing scan and generating insights...');
        ...
        try {
            ...
            logger_1.logger.info(`Scan complete: ${this.activeScan.vulnerabilities.length} vulnerabilities found in ${this.activeScan.totalDuration}ms`);
        }
        catch (error) {
            logger_1.logger.error(`Failed to complete scan: ${error.message}`);
        }
    }
    generateSimplifiedInsights() {
        if (!this.activeScan)
            return [];
        return [
            {
    ...
    updateScanProgress(phase, progress, message, currentEndpoint) {
        if (!this.activeScan)
            return;
        ...
        logger_1.logger.info(`Scan progress: ${newProgress.toFixed(1)}% - ${enhancedMessage}`);
    ...
    getPhaseDetails(phase, progress) {
        const phaseRanges = {
        ...
        const phaseProgress = Math.min(100, Math.max(0, ((progress - currentPhase.start) / (currentPhase.end - currentPhase.start)) * 100));
        return {
    ...
    broadcastScanUpdate(eventType, data) {
    ...
    inferBusinessCriticality(endpoint) {
        const highRiskPaths = ['/admin', '/api/admin', '/users', '/auth', '/payment', '/billing'];
        const mediumRiskPaths = ['/api', '/data', '/profile', '/settings'];
        const path = endpoint.toLowerCase();
        if (highRiskPaths.some(risk => path.includes(risk)))
            return 'HIGH';
        if (mediumRiskPaths.some(risk => path.includes(risk)))
            return 'MEDIUM';
        return 'LOW';
    }
    inferDataClassification(endpoint) {
        const confidentialPaths = ['/admin', '/payment', '/billing', '/users', '/auth'];
        const internalPaths = ['/api', '/data', '/profile'];
        const path = endpoint.toLowerCase();
        if (confidentialPaths.some(conf => path.includes(conf)))
            return 'CONFIDENTIAL';
        if (internalPaths.some(internal => path.includes(internal)))
            return 'INTERNAL';
        return 'PUBLIC';
    }
    convertParameterInfoToParameter(parameterInfos) {
    ...
    mapDataTypeToParameterType(dataType) {
        switch (dataType) {
            case 'integer':
            case 'number':
                return 'number';
    ...
    cancelScan(scanId) {
        if (this.activeScan?.scanId === scanId) {
            logger_1.logger.info(`Cancelling scan: ${scanId}`);
            this.activeScan = null;
...
exports.RealTimeApiScanner = RealTimeApiScanner;
//# sourceMappingURL=realTimeApiScanner.js.map