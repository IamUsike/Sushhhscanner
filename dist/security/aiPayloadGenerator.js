...
exports.AIPayloadGenerator = void 0;
...
class AIPayloadGenerator {
    ...
    async generatePayloads(parameter, context) {
        ...
        try {
            ...
            if (context.applicationContext?.framework) {
                const frameworkPayloads = this.generateFrameworkSpecificPayloads(parameter, context.applicationContext.framework);
            ...
            logger_1.logger.error(`Payload generation failed: ${error.message}`);
            return [];
        }
    }
    generateStaticPayloads(parameter) {
        const payloads = [];
    ...
    generateContextAwarePayloads(parameter, context) {
        const payloads = [];
    ...
    async generateMLEnhancedPayloads(parameter, context) {
        ...
        try {
            ...
            logger_1.logger.warn(`ML payload generation error: ${error.message}`);
        }
        return payloads;
    }
    generateFrameworkSpecificPayloads(parameter, framework) {
        const payloads = [];
        if (this.frameworkPayloads[framework]) {
            const frameworkConfig = this.frameworkPayloads[framework];
    ...
    generateMutationPayloads(parameter, basePayloads) {
        ...
        const mutations = [
            ...
            (payload) => this.randomCase(payload),
            // Encoding mutations
            (payload) => encodeURIComponent(payload),
            (payload) => this.doubleEncode(payload),
    ...
    async analyzePayloadEffectiveness(parameter, payload, response, baseline) {
        let effectiveness = 0;
        const reasons = [];
...
exports.AIPayloadGenerator = AIPayloadGenerator;
//# sourceMappingURL=aiPayloadGenerator.js.map