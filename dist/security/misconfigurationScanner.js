...
exports.MisconfigurationScanner = void 0;
const misconfigurationDetector_1 = require("./misconfigurationDetector");
...
class MisconfigurationScanner {
    constructor(options = {}) {
        ...
        this.detector = new misconfigurationDetector_1.MisconfigurationDetector(options);
        this.configAnalyzer = new configurationAnalyzer_1.ConfigurationAnalyzer();
    }
    async scanTarget(target, progressCallback) {
        const scanStartTime = new Date().toISOString();
        logger_1.logger.info(`Starting comprehensive misconfiguration scan for: ${target}`);
        if (progressCallback) {
        ...
        try {
            ...
            const scanResult = this.generateScanResult(target, scanStartTime, scanEndTime, scanDuration, allFindings);
            if (progressCallback) {
                progressCallback('✅ Misconfiguration scan completed successfully!');
            }
            logger_1.logger.info(`Misconfiguration scan completed. Found ${allFindings.length} total issues.`);
            return scanResult;
        }
        catch (error) {
    ...
    async analyzeConfigurationFiles(target) {
        const findings = [];
        try {
            ...
            const configFiles = [
                ...
                { path: '/composer.json', type: 'json' },
                { path: '/web.config', type: 'xml' },
                { path: '/docker-compose.yml', type: 'yaml' },
                { path: '/docker-compose.yaml', type: 'yaml' },
                { path: '/config.yml', type: 'yaml' },
                { path: '/config.yaml', type: 'yaml' },
                { path: '/database.yml', type: 'yaml' },
                { path: '/.github/workflows/main.yml', type: 'yaml' }
            ];
            ...
            logger_1.logger.warn(`Configuration file analysis failed: ${error.message}`);
        }
        return findings;
    }
    async analyzeSwaggerConfigurations(target) {
        const findings = [];
        try {
                                        ...
                                        for (const [method, methodObj] of Object.entries(pathObj)) {
                                            if (['get', 'post', 'put', 'delete', 'patch', 'options', 'head'].includes(method)) {
                                                endpoints.push({
                                                    ...
                                                    authentication: methodObj?.security || swaggerData.security,
            ...
            logger_1.logger.warn(`Swagger configuration analysis failed: ${error.message}`);
        }
        return findings;
    }
    async analyzeCloudConfigurations(target) {
        const findings = [];
        try {
            ...
            logger_1.logger.warn(`Cloud configuration analysis failed: ${error.message}`);
        }
        return findings;
    }
    generateScanResult(target, scanStartTime, scanEndTime, scanDuration, findings) {
    ...
    generateRecommendations(findings, severityBreakdown) {
    ...
    generateExecutiveSummary(target, findings, severityBreakdown) {
        ...
        // Top categories
        const topCategories = Object.entries(findings.reduce((acc, f) => {
    ...
    calculateOverallRiskLevel(severityBreakdown) {
        if (severityBreakdown.CRITICAL > 0)
            return 'HIGH RISK';
        if (severityBreakdown.HIGH >= 3)
            return 'HIGH RISK';
        if (severityBreakdown.HIGH > 0 || severityBreakdown.MEDIUM >= 5)
            return 'MEDIUM RISK';
        if (severityBreakdown.MEDIUM > 0 || severityBreakdown.LOW >= 3)
            return 'LOW RISK';
        return 'MINIMAL RISK';
    }
    ...
    async exportReport(scanResult, format = 'json') {
        switch (format) {
            case 'json':
...
exports.MisconfigurationScanner = MisconfigurationScanner;
//# sourceMappingURL=misconfigurationScanner.js.map