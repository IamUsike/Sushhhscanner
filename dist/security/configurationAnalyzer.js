...
class ConfigurationAnalyzer {
    ...
    async analyzeConfiguration(configFile) {
        logger_1.logger.info(`Analyzing configuration file: ${configFile.filename}`);
        const results = [];
        try {
            ...
            logger_1.logger.error(`Failed to analyze configuration: ${error.message}`);
            return [];
        }
    }
    async analyzeAPIConfiguration(apiConfig, baseUrl) {
        const results = [];
        try {
            // Check for endpoints without authentication
            const unauthEndpoints = apiConfig.endpoints.filter(ep => !ep.authentication || ep.authentication.required === false);
            ...
            logger_1.logger.error(`Failed to analyze API configuration: ${error.message}`);
            return [];
        }
    }
    ...
    checkExposedCredentials(configFile) {
        const results = [];
    ...
    checkDangerousConfigurations(configFile) {
        const results = [];
    ...
    checkWeakCredentials(configFile) {
        const results = [];
    ...
    analyzeJSONConfiguration(configFile, parsedConfig) {
        const results = [];
        if (!parsedConfig)
            return results;
        try {
            ...
            if (parsedConfig.dependencies) {
                const insecurePackages = ['express', 'mysql', 'mongodb'].filter(pkg => parsedConfig.dependencies[pkg] && parsedConfig.dependencies[pkg].includes('*'));
                if (insecurePackages.length > 0) {
                    results.push({
                        ...
                        evidence: {
                            file: configFile.filename,
                            content: JSON.stringify(Object.fromEntries(insecurePackages.map(pkg => [pkg, parsedConfig.dependencies[pkg]])), null, 2),
                            url: configFile.url
                        },
                        cwe: 'CWE-1104',
            ...
            logger_1.logger.warn(`Failed to analyze JSON configuration: ${error.message}`);
        }
        return results;
    }
    analyzeYAMLConfiguration(configFile, parsedConfig) {
        const results = [];
        if (!parsedConfig)
            return results;
        try {
            ...
            logger_1.logger.warn(`Failed to analyze YAML configuration: ${error.message}`);
        }
        return results;
    }
    analyzeEnvConfiguration(configFile) {
        const results = [];
        try {
            const lines = configFile.content.split('\n');
            for (const line of lines) {
            ...
            logger_1.logger.warn(`Failed to analyze .env configuration: ${error.message}`);
        }
        return results;
    }
    analyzeSwaggerSecurity(swaggerConfig, baseUrl) {
        const results = [];
        try {
            ...
            if (swaggerConfig.securityDefinitions) {
                const basicAuthSchemes = Object.entries(swaggerConfig.securityDefinitions)
                    .filter(([_, scheme]) => scheme.type === 'basic');
            ...
            logger_1.logger.warn(`Failed to analyze Swagger security: ${error.message}`);
        }
        return results;
    }
    extractMatchingLine(content, pattern) {
