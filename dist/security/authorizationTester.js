...
exports.AuthorizationTester = void 0;
...
class AuthorizationTester {
    ...
    async testAuthorization(endpoint, userContexts, options = {}) {
        const { timeout = 10000, includeDestructive = false, testIDOR = true, testPathTraversal = true } = options;
        ...
        try {
            ...
            if (missingACResult)
                results.push(missingACResult);
            // 2. Test horizontal privilege escalation (if multiple user contexts)
            if (userContexts.length >= 2) {
                const horizontalResults = await this.testHorizontalPrivilegeEscalation(endpoint, userContexts, timeout);
                results.push(...horizontalResults);
            }
            // 3. Test vertical privilege escalation
            const verticalResults = await this.testVerticalPrivilegeEscalation(endpoint, userContexts, timeout);
    ...
    async testMissingAccessControl(endpoint, timeout) {
        try {
    ...
    async testHorizontalPrivilegeEscalation(endpoint, userContexts, timeout) {
        const results = [];
    ...
    async testVerticalPrivilegeEscalation(endpoint, userContexts, timeout) {
        const results = [];
    ...
    async testIDOR(endpoint, userContexts, timeout) {
        const results = [];
    ...
    async testRoleBypass(endpoint, userContexts, timeout) {
        const results = [];
    ...
    async testPathTraversal(endpoint, timeout) {
        const results = [];
    ...
    async testAdminAccess(endpoint, userContexts, timeout) {
        const results = [];
    ...
    containsAdminData(data) {
        ...
        return adminIndicators.some(indicator => dataStr.toLowerCase().includes(indicator));
    }
    containsSystemFiles(data) {
        if (!data)
            return false;
        const dataStr = typeof data === 'string' ? data : JSON.stringify(data);
        ...
        return systemIndicators.some(indicator => dataStr.toLowerCase().includes(indicator));
    }
...
exports.AuthorizationTester = AuthorizationTester;
//# sourceMappingURL=authorizationTester.js.map