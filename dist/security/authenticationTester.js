...
exports.AuthenticationTester = void 0;
...
class AuthenticationTester {
    ...
    async testAuthentication(endpoint, options = {}) {
        const { timeout = 10000, includeDestructive = false, maxBruteForceAttempts = 5 } = options;
        ...
        try {
            ...
            if (includeDestructive && maxBruteForceAttempts > 0) {
                const bruteForceResult = await this.testBruteForceProtection(endpoint, authInfo, timeout, maxBruteForceAttempts);
                if (bruteForceResult)
    ...
    async detectAuthenticationMethod(endpoint, timeout) {
        const authInfo = {
            type: 'none',
            location: 'header',
            detected: false,
    ...
    async testNoAuthentication(endpoint, timeout) {
        try {
            const response = await this.makeRequest(endpoint, {}, timeout);
    ...
    async testAuthenticationBypass(endpoint, authInfo, timeout) {
        const results = [];
    ...
    async testWeakAuthentication(endpoint, authInfo, timeout) {
        const results = [];
    ...
    async testTokenVulnerabilities(endpoint, authInfo, timeout) {
        const results = [];
    ...
    async testBruteForceProtection(endpoint, authInfo, timeout, maxAttempts) {
        if (authInfo.type !== 'basic') {
    ...
    async testInformationDisclosure(endpoint, timeout) {
        const results = [];
        try {
    ...
    containsSuccessfulData(data) {
        ...
        const hasSuccess = successIndicators.some(indicator => dataStr.toLowerCase().includes(indicator));
        const hasError = errorIndicators.some(indicator => dataStr.toLowerCase().includes(indicator));
    ...
    buildRequestString(endpoint, headers, params) {
...
exports.AuthenticationTester = AuthenticationTester;
//# sourceMappingURL=authenticationTester.js.map