...
const enhancedRiskScoringEngine_1 = require("../ai/enhancedRiskScoringEngine");
...
router.post('/initialize', async (req, res) => {
    try {
        logger_1.logger.info('Initializing enhanced ML engine via API...');
        if (!enhancedEngine) {
            enhancedEngine = new enhancedRiskScoringEngine_1.EnhancedRiskScoringEngine();
            await enhancedEngine.initialize();
            logger_1.logger.info('Enhanced ML engine initialized successfully');
        }
        ...
        logger_1.logger.error(`Enhanced ML initialization failed: ${error.message}`);
        res.status(500).json({
            success: false,
...
router.post('/bulk-risk-score', async (req, res) => {
    try {
        ...
        if (!vulnerabilities || !Array.isArray(vulnerabilities)) {
            return res.status(400).json({
                success: false,
                message: 'Vulnerabilities array is required'
            });
        }
        if (!enhancedEngine) {
            // Fallback calculation
            const totalScore = vulnerabilities.reduce((sum, v) => {
                return sum + calculateStandardRiskScore(v);
            }, 0);
            return res.json({
                success: true,
                riskScore: {
        ...
        const riskScores = await Promise.all(vulnerabilities.map(vuln => enhancedEngine.calculateEnhancedRiskScore(vuln)));
        // Aggregate scores
        ...
        logger_1.logger.error(`Bulk risk scoring failed: ${error.message}`);
        res.status(500).json({
            success: false,
...
router.get('/metrics', (req, res) => {
    try {
        ...
        logger_1.logger.error(`Failed to get ML metrics: ${error.message}`);
        res.status(500).json({
            success: false,
...
// Helper functions
function calculateStandardRiskScore(vulnerability) {
...
function getCVSSSeverity(baseScore) {
    if (baseScore >= 9.0)
        return 'CRITICAL';
    if (baseScore >= 7.0)
        return 'HIGH';
