...
export type SecurityTestType = 'auth' | 'injection' | 'exposure' | 'config' | 'rate-limiting' | 'headers' | 'business-logic' | 'data-exposure';
export type VulnerabilitySeverity = 'CRITICAL' | 'HIGH' | 'MEDIUM' | 'LOW' | 'INFO';
export type VulnerabilityType = 'NO_AUTHENTICATION' | 'WEAK_AUTHENTICATION' | 'BROKEN_ACCESS_CONTROL' | 'SQL_INJECTION' | 'NOSQL_INJECTION' | 'XSS' | 'COMMAND_INJECTION' | 'LDAP_INJECTION' | 'PII_EXPOSURE' | 'SENSITIVE_DATA_EXPOSURE' | 'MISSING_SECURITY_HEADERS' | 'CORS_MISCONFIGURATION' | 'RATE_LIMITING_BYPASS' | 'BUSINESS_LOGIC_FLAW' | 'INFORMATION_DISCLOSURE' | 'FILE_EXPOSURE' | 'DEBUG_MODE_ENABLED' | 'ADMIN_PANEL_EXPOSED' | 'MASS_ASSIGNMENT' | 'RACE_CONDITION' | 'DIRECTORY_LISTING_ENABLED' | 'SERVER_INFO_DISCLOSURE' | 'X_POWERED_BY_HEADER_DISCLOSURE' | 'DETAILED_ERROR_MESSAGES' | 'PERMISSIVE_CORS_WITH_CREDENTIALS' | 'PERMISSIVE_CORS' | 'CSP_UNSAFE_INLINE' | 'CSP_UNSAFE_EVAL' | 'CSP_WEAK_DEFAULT_SRC' | 'CSP_MISSING' | 'MISSING_HSTS' | 'TLS_CERT_HOSTNAME_MISMATCH' | 'TLS_CERT_UNTRUSTED' | 'EXPOSED_SENSITIVE_PATH_IN_ROBOTS_OR_SITEMAP' | 'MISSING_SECURE_COOKIE_FLAG' | 'MISSING_HTTPONLY_COOKIE_FLAG' | 'MISSING_SAMESITE_COOKIE_FLAG' | 'UNSAFE_HTTP_METHOD_ALLOWED' | 'HTTP_METHOD_ALLOWED_WITH_AUTH';
...
export type ScanStatus = 'pending' | 'discovering' | 'testing' | 'analyzing' | 'completed' | 'failed' | 'cancelled';
