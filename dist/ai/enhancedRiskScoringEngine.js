...
exports.EnhancedRiskScoringEngine = void 0;
...
class EnhancedRiskScoringEngine {
    ...
    async initialize() {
        logger_1.logger.info('Initializing Enhanced AI/ML Risk Scoring Engine with CVSS Integration...');
        try {
            ...
            logger_1.logger.info('Enhanced Risk Scoring Engine initialized successfully');
        }
        catch (error) {
            logger_1.logger.error(`Failed to initialize Enhanced Risk Scoring Engine: ${error.message}`);
            throw error;
        }
    }
    async calculateEnhancedRiskScore(vulnerability) {
        ...
        logger_1.logger.info(`Calculating enhanced AI/ML risk score for ${vulnerability.type} vulnerability`);
        try {
            ...
            logger_1.logger.error(`Enhanced risk scoring failed: ${error.message}`);
            // Fallback to rule-based scoring with CVSS
            return await this.calculateEnhancedFallbackScore(vulnerability);
        }
    }
    async createEnhancedModels() {
        logger_1.logger.info('Creating enhanced ML models for risk scoring...');
    ...
    async trainEnhancedModels() {
        logger_1.logger.info('Training enhanced ML models with CVSS integration...');
        try {
            ...
            logger_1.logger.info('Enhanced ML models trained successfully');
        }
        catch (error) {
            logger_1.logger.error(`Enhanced model training failed: ${error.message}`);
            throw error;
        }
    }
    extractEnhancedFeatures(vulnerability, cvssMetrics) {
        const features = [];
    ...
    async calculateCVSSMetrics(vulnerability) {
    ...
    calculateCVSSBaseScore(metrics) {
        ...
        if (metrics.attackVector === 'NETWORK')
            exploitability *= 0.85;
        if (metrics.privilegesRequired === 'NONE')
            exploitability *= 0.85;
        if (metrics.userInteraction === 'NONE')
            exploitability *= 0.85;
    ...
    getAttackVectorScore(attackVector) {
        switch (attackVector) {
    ...
    async predictCVSSRisk(features) {
        if (!this.cvssModel) {
            return 0.5; // Fallback
        }
        const input = tf.tensor2d([features.slice(4, 19)]); // CVSS-specific features
        const prediction = this.cvssModel.predict(input);
        const score = await prediction.data();
        input.dispose();
        prediction.dispose();
        return score[0];
    }
    calculateCVSSAdjustedScore(baseScore, cvssMetrics) {
    ...
    async generateEnhancedPredictions(features, overallScore, cvssMetrics) {
    ...
    generateEnhancedRecommendations(vulnerability, riskScore, predictions, cvssMetrics) {
    ...
    generateCVSSRemediation(vulnerability, cvssMetrics) {
        const remediations = [];
    ...
    async generateEnhancedTrainingData(samples) {
        ...
        for (let i = 0; i < samples; i++) {
            const vuln = {
                ...
                attackVector: ['NETWORK', 'ADJACENT_NETWORK', 'LOCAL'][Math.floor(Math.random() * 3)],
                privilegesRequired: ['NONE', 'LOW', 'HIGH'][Math.floor(Math.random() * 3)],
                userInteraction: ['NONE', 'REQUIRED'][Math.floor(Math.random() * 2)],
                scope: ['UNCHANGED', 'CHANGED'][Math.floor(Math.random() * 2)],
                confidentialityImpact: ['NONE', 'LOW', 'HIGH'][Math.floor(Math.random() * 3)],
                integrityImpact: ['NONE', 'LOW', 'HIGH'][Math.floor(Math.random() * 3)],
                availabilityImpact: ['NONE', 'LOW', 'HIGH'][Math.floor(Math.random() * 3)],
    ...
    calculateContextualRisk(vulnerability) {
        ...
        if (vulnerability.businessCriticality === 'HIGH')
            risk += 0.3;
        else if (vulnerability.businessCriticality === 'MEDIUM')
            risk += 0.15;
        // Data classification
        if (vulnerability.dataClassification === 'CONFIDENTIAL')
            risk += 0.2;
        else if (vulnerability.dataClassification === 'INTERNAL')
            risk += 0.1;
        // User access
        if (vulnerability.userAccess === 'EXTERNAL')
            risk += 0.2;
        else if (vulnerability.userAccess === 'INTERNAL')
            risk += 0.1;
    ...
    calculateTemporalRisk(vulnerability) {
        ...
        if (vulnerability.exploitability && vulnerability.exploitability > 0.7)
            risk += 0.2;
        if (vulnerability.impact && vulnerability.impact > 0.7)
            risk += 0.2;
        // Attack complexity
    ...
    async calculateEnhancedEnsembleScore(features, components) {
        if (!this.ensembleModel) {
            // Weighted average fallback with CVSS
            return (components.severity * 0.20 +
    ...
    calculateModelConfidence(features, riskScore, cvssMetrics) {
    ...
    async calculateEnhancedFallbackScore(vulnerability) {
    ...
    async saveModels(basePath) {
        logger_1.logger.info('Saving enhanced ML models...');
        if (this.severityModel)
            await this.severityModel.save(`file://${basePath}/severity_model`);
        if (this.cvssModel)
            await this.cvssModel.save(`file://${basePath}/cvss_model`);
        if (this.anomalyDetectionModel)
            await this.anomalyDetectionModel.save(`file://${basePath}/anomaly_model`);
        if (this.ensembleModel)
            await this.ensembleModel.save(`file://${basePath}/ensemble_model`);
        logger_1.logger.info('Enhanced ML models saved successfully');
    }
    async loadModels(basePath) {
        logger_1.logger.info('Loading enhanced ML models...');
        try {
            ...
            this.isInitialized = true;
            logger_1.logger.info('Enhanced ML models loaded successfully');
        }
        catch (error) {
            logger_1.logger.warn(`Failed to load enhanced models: ${error.message}`);
        }
    }
}
exports.EnhancedRiskScoringEngine = EnhancedRiskScoringEngine;
//# sourceMappingURL=enhancedRiskScoringEngine.js.map