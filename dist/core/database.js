...
class Database {
    ...
    async initialize() {
        try {
            ...
            logger_1.logger.info(`Database initialized successfully (${this.dbType})`);
        }
        catch (error) {
        ...
        return new Promise((resolve, reject) => {
            this.sqliteDb = new sqlite3_1.default.Database(dbPath, (err) => {
                if (err) {
                    ...
                    logger_1.logger.info(`SQLite database connected: ${dbPath}`);
                    resolve();
                }
            });
        });
    ...
    async initializePostgreSQL() {
        ...
        this.pgClient = new pg_1.Client({ connectionString });
    ...
    async getReportsByScanId(scanId) {
        const sql = this.dbType === 'sqlite' ?
